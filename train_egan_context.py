    ...
    for i, data in enumerate(dataloader, 0):
        ...
        if i % 100 == 0:
            ...
            vutils.save_image(fake.data,
                    '%s/context_E_fake_samples_epoch_%03d.png' % ('log', epoch),
                    normalize=True)


    # do checkpointing
torch.save(G.state_dict(), '%s/context_netG_epoch_%d.pth' % ('log', epoch))
torch.save(SND1.state_dict(), '%s/context_netD1_epoch_%d.pth' % ('log', epoch)) 
torch.save(SND2.state_dict(), '%s/context_netD2_epoch_%d.pth' % ('log', epoch)) 
torch.save(SND3.state_dict(), '%s/context_netD3_epoch_%d.pth' % ('log', epoch)) 
torch.save(E.state_dict(), '%s/context_netE_epoch_%d.pth' % ('log', epoch)) 
